...
import PrevNextBtn from "@/components/Portfolio/Case_Study/PrevNextBtn";
...
export default async function ProjectsPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    <article id="pageContainerr" className="flex flex-col justify-center items-center min-h-screen md:p-8 px-4 py-8">
      <section id="sectionContainer" className="relative min-h-screen py-5 md:py-10 flex justify-center items-center flex-col gap-4">
        ...
        <PrevNextBtn next={next} />
        
        <div id="backgroundCard" className="relative z-10 md:max-w-7xl bg-[#F9FAF9] md:px-12 md:pb-12 rounded-2xl shadow-2xl">
          {/* Project image */}
          <div className="relative w-full mb-8 max-md:rounded-lg md:rounded-b-lg overflow-hidden" style={{aspectRatio: "16/9"}}>
            <Image
              src={project.image}
              alt={project.title}
              fill
